...
/**
 * When press "c" on keyboard, store the current tetromino into the hold frame
 * mention: if hold frame has no value, can hold the current falling tetromino
 * mention2: used to switch the current falling tetromino with one in hold
 * mention3: if current falling tetromino is from hold frame,cannot be switch again
 * @function
 * @memberof Tetris
 * @param {Tetris.Game} / game main object.
 * @returns {Tetris.Game} 
 */
Tetris.hold = function (game) {
    if (game.held_type == game.current_tetromino.block_type) {
        return game;
    }
    ...
    held_type = game.current_tetromino.block_type;
    game.held_type = held_type;
    game.position = [5, 0];
    return game;
}

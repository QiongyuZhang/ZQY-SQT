...
describe("Hold", function () {
    ...
    it(
        ...
        function () {
            const initial_game = Tetris.new_game();
            let next_game = initial_game.hold(initial_game)
            let current = next_game.current_tetromino;
            let next_game2 = initial_game.hold(next_game)
            if (!R.equals(current.block_type, next_game2.current_tetromino.block_type)) {
                throw new Error(
                    `The inital and final tetrominos do not match
                    current_tetromino1: ${JSON.stringify(current)}
                    current_tetromino2:   ${JSON.stringify(next_game2.current_tetromino)}`
                );
            }
        }
    );
